...
import com.wechatmall.api.service.IExpressService;
...
@Tag(name = "快递管理", description = "相关的API接口")
@Validated
public class ExpressController {
    ...
    public ApiResultResponse<List<Express>> list() {
        List<Express> result = expressService.list();
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<List<Express>> page(
        ...
        IPage<Express> result = expressService.page(page);
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<List<Express>> pageAndOrder(
            @RequestParam(defaultValue = "1") @Positive(message = "页码必须大于0") Long current,
            @RequestParam(defaultValue = "10") @Positive(message = "每页数量必须大于0") Long size,
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<Express> getById(@PathVariable @NotNull(message = "ID不能为空") String id) {
        Express result = expressService.getById(id);
        if (result == null) {
    ...
    public ApiResultResponse<Boolean> save(
            @RequestBody @Valid Express express) {
        boolean result = expressService.save(express);
    ...
    public ApiResultResponse<Boolean> updateById(
            ...
            @RequestBody @Valid Express express) {
        express.setId(id);
        boolean result = expressService.updateById(express);
    ...
    public ApiResultResponse<Boolean> removeById(@PathVariable @NotNull(message = "ID不能为空") Long id) {
        boolean result = expressService.removeById(id);
    ...
    public ApiResultResponse<Boolean> removeByIds(@PathVariable @NotNull(message = "ids不能为空") String ids) {
        ...
        boolean result = expressService.removeByIds(Arrays.asList(ids.split(",")));
