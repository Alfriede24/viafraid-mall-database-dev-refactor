package com.wechatmall.api.controller.message;

import com.wechatmall.api.service.ICostumServiceService;
import com.wechatmall.api.pojo.entity.CostumService;
 ...
 */
@RestController
@RequestMapping("/costum-service")
@Tag(name = "客服管理", description = "相关的API接口")
@Validated
public class CostumServiceController {

    @Autowired
    private ICostumServiceService costumServiceService;
    ...
    public ApiResultResponse<List<CostumService>> list() {
        List<CostumService> result = costumServiceService.list();
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<List<CostumService>> page(
        ...
        Page<CostumService> page = new Page<>(current, size);
        IPage<CostumService> result = costumServiceService.page(page);
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<List<CostumService>> pageAndOrder(
            @RequestParam(defaultValue = "1") @Positive(message = "页码必须大于0") Long current,
            @RequestParam(defaultValue = "10") @Positive(message = "每页数量必须大于0") Long size,
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<CostumService> getById(@PathVariable @NotNull(message = "ID不能为空") String id) {
        CostumService result = costumServiceService.getById(id);
        if (result == null) {
    ...
    public ApiResultResponse<Boolean> save(
            @RequestBody @Valid CostumService costumService) {
        boolean result = costumServiceService.save(costumService);
    ...
    public ApiResultResponse<Boolean> updateById(
            ...
            @RequestBody @Valid CostumService costumService) {
        costumService.setId(id);
        boolean result = costumServiceService.updateById(costumService);
    ...
    public ApiResultResponse<Boolean> removeById(@PathVariable @NotNull(message = "ID不能为空") Long id) {
        boolean result = costumServiceService.removeById(id);
    ...
    public ApiResultResponse<Boolean> removeByIds(@PathVariable @NotNull(message = "ids不能为空") String ids) {
        ...
        boolean result = costumServiceService.removeByIds(Arrays.asList(ids.split(",")));
