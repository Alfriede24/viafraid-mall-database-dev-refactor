...
import com.wechatmall.api.service.IUserInformationService;
 ...
 */
@RestController
@RequestMapping("/client/user-information")
@Tag(name = "用户信息管理", description = "相关的API接口")
@Validated
public class UserInformationController {
    ...
    public ApiResultResponse<List<UserInformation>> list() {
        List<UserInformation> result = userInformationService.list();
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<List<UserInformation>> page(
        ...
        Page<UserInformation> page = new Page<>(current, size);
        IPage<UserInformation> result = userInformationService.page(page);
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<List<UserInformation>> pageAndOrder(
            @RequestParam(defaultValue = "1") @Positive(message = "页码必须大于0") Long current,
            @RequestParam(defaultValue = "10") @Positive(message = "每页数量必须大于0") Long size,
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<UserInformation> getById(@PathVariable @NotNull(message = "ID不能为空") String id) {
        UserInformation result = userInformationService.getById(id);
        if (result == null) {
    ...
    public ApiResultResponse<Boolean> save(
        ...
        boolean result = userInformationService.save(userInformation);
    ...
    public ApiResultResponse<Boolean> updateById(
        ...
        boolean result = userInformationService.updateById(userInformation);
    ...
    public ApiResultResponse<Boolean> removeById(@PathVariable @NotNull(message = "ID不能为空") Long id) {
        boolean result = userInformationService.removeById(id);
    ...
    public ApiResultResponse<Boolean> removeByIds(@PathVariable @NotNull(message = "ids不能为空") String ids) {
        ...
        boolean result = userInformationService.removeByIds(Arrays.asList(ids.split(",")));
