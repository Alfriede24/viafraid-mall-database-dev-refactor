package com.wechatmall.api.controller.merchant;
...
@RestController
@RequestMapping("/root")
@Tag(name = "商家管理", description = "相关的API接口")
@Validated
public class RootController {
    ...
    public ApiResultResponse<List<Root>> list() {
        List<Root> result = rootService.list();
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<List<Root>> page(
        ...
        Page<Root> page = new Page<>(current, size);
        IPage<Root> result = rootService.page(page);
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<List<Root>> pageAndOrder(
            @RequestParam(defaultValue = "1") @Positive(message = "页码必须大于0") Long current,
            @RequestParam(defaultValue = "10") @Positive(message = "每页数量必须大于0") Long size,
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<Root> getById(@PathVariable @NotNull(message = "ID不能为空") String id) {
        Root result = rootService.getById(id);
        if (result == null) {
    ...
    public ApiResultResponse<Boolean> save(
        ...
        boolean result = rootService.save(root);
    ...
    public ApiResultResponse<Boolean> updateById(
            ...
            @RequestBody @Valid Root root) {
        root.setRootId(id);
        boolean result = rootService.updateById(root);
    ...
    public ApiResultResponse<Boolean> removeById(@PathVariable @NotNull(message = "ID不能为空") Long id) {
        boolean result = rootService.removeById(id);
    ...
    public ApiResultResponse<Boolean> removeByIds(@PathVariable @NotNull(message = "ids不能为空") String ids) {
        ...
        boolean result = rootService.removeByIds(Arrays.asList(ids.split(",")));
