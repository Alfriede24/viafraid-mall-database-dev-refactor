package com.wechatmall.api.controller.client;
 ...
 */
@RestController
@RequestMapping("/client/user-order")
@Tag(name = "用户订单管理", description = "相关的API接口")
@Validated
public class UserOrderController {
    ...
    public ApiResultResponse<List<UserOrder>> list() {
        List<UserOrder> result = userOrderService.list();
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<List<UserOrder>> page(
        ...
        IPage<UserOrder> result = userOrderService.page(page);
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<List<UserOrder>> pageAndOrder(
            @RequestParam(defaultValue = "1") @Positive(message = "页码必须大于0") Long current,
            @RequestParam(defaultValue = "10") @Positive(message = "每页数量必须大于0") Long size,
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<UserOrder> getById(@PathVariable @NotNull(message = "ID不能为空") String id) {
        UserOrder result = userOrderService.getById(id);
        if (result == null) {
    ...
    public ApiResultResponse<Boolean> save(
        ...
        boolean result = userOrderService.save(userOrder);
    ...
    public ApiResultResponse<Boolean> updateById(
            ...
            @RequestBody @Valid UserOrder userOrder) {
        userOrder.setId(id);
        boolean result = userOrderService.updateById(userOrder);
    ...
    public ApiResultResponse<Boolean> removeById(@PathVariable @NotNull(message = "ID不能为空") Long id) {
        boolean result = userOrderService.removeById(id);
    ...
    public ApiResultResponse<Boolean> removeByIds(@PathVariable @NotNull(message = "ids不能为空") String ids) {
        ...
        boolean result = userOrderService.removeByIds(Arrays.asList(ids.split(",")));
    ...
    @Operation(
            summary = "查询详情",
            description = "根据用户ID查询用户订单详情",
            responses = {
                    @ApiResponse(responseCode = "200", description = "查询详情成功",
                            content = @Content(mediaType = "application/json", schema = @Schema(implementation = ApiResultResponse.class),
                                    examples = @ExampleObject(value = "{\"code\":200,\"message\":\"查询详情成功\",\"data\":true,\"timestamp\":\"2024-01-01T12:00:00\"}"))),
                    @ApiResponse(responseCode = "400", description = "请求参数错误"),
                    @ApiResponse(responseCode = "500", description = "服务器内部错误")
            }
    )
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<List<UserOrderVo>> selectUserOrderByUserId(@PathVariable @NotNull(message = "用户ID不能为空") String userId) {
          return ApiResultResponse.list(this.userOrderService.selectUserOrderByUserId(userId));
    }
}
