...
import com.wechatmall.api.service.IShopItemMainService;
import com.wechatmall.api.pojo.entity.ShopItemMain;
 ...
 */
@RestController
@RequestMapping("/common/shop-item-main")
@Tag(name = "商品项关联管理", description = "相关的API接口")
@Validated
public class ShopItemMainController {

    @Autowired
    private IShopItemMainService shopItemMainService;
    ...
    public ApiResultResponse<List<ShopItemMain>> list() {
        List<ShopItemMain> result = shopItemMainService.list();
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<List<ShopItemMain>> page(
        ...
        Page<ShopItemMain> page = new Page<>(current, size);
        IPage<ShopItemMain> result = shopItemMainService.page(page);
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<List<ShopItemMain>> pageAndOrder(
            @RequestParam(defaultValue = "1") @Positive(message = "页码必须大于0") Long current,
            @RequestParam(defaultValue = "10") @Positive(message = "每页数量必须大于0") Long size,
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<ShopItemMain> getById(@PathVariable @NotNull(message = "ID不能为空") String id) {
        ShopItemMain result = shopItemMainService.getById(id);
        if (result == null) {
    ...
    public ApiResultResponse<Boolean> save(
            @RequestBody @Valid ShopItemMain shopItemMain) {
        boolean result = shopItemMainService.save(shopItemMain);
    ...
    public ApiResultResponse<Boolean> updateById(
            ...
            @RequestBody @Valid ShopItemMain shopItemMain) {
        shopItemMain.setId(id);
        boolean result = shopItemMainService.updateById(shopItemMain);
    ...
    public ApiResultResponse<Boolean> removeById(@PathVariable @NotNull(message = "ID不能为空") Long id) {
        boolean result = shopItemMainService.removeById(id);
    ...
    public ApiResultResponse<Boolean> removeByIds(@PathVariable @NotNull(message = "ids不能为空") String ids) {
        ...
        boolean result = shopItemMainService.removeByIds(Arrays.asList(ids.split(",")));
