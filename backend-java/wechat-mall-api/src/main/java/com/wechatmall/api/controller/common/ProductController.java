...
@Tag(name = "商品管理", description = "相关的API接口")
@Validated
public class ProductController {
    ...
    public ApiResultResponse<List<Product>> list() {
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<List<Product>> page(
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<List<Product>> pageAndOrder(
            @RequestParam(defaultValue = "1") @Positive(message = "页码必须大于0") Long current,
            @RequestParam(defaultValue = "10") @Positive(message = "每页数量必须大于0") Long size,
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<Product> getById(@PathVariable @NotNull(message = "ID不能为空") String id) {
    ...
    public ApiResultResponse<Boolean> removeByIds(@PathVariable @NotNull(message = "ids不能为空") String ids) {
        ...
        boolean result = productService.removeByIds(Arrays.asList(ids.split(",")));
