...
import com.wechatmall.api.service.IAddressService;
...
@Tag(name = "地址管理", description = "相关的API接口")
@Validated
public class AddressController {
    ...
    public ApiResultResponse<List<Address>> list() {
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<List<Address>> page(
        ...
        IPage<Address> result = addressService.page(page);
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<List<Address>> pageAndOrder(
            @RequestParam(defaultValue = "1") @Positive(message = "页码必须大于0") Long current,
            @RequestParam(defaultValue = "10") @Positive(message = "每页数量必须大于0") Long size,
    ...
    @ApiOperationSupport(author = "zpc")
    public ApiResultResponse<Address> getById(@PathVariable @NotNull(message = "ID不能为空") String id) {
